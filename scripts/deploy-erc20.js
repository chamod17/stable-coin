...
async function main() {
  console.log("Deploying CNY Stablecoin (ERC20) to network:", hre.network.name);

  // Get admin address from environment or use deployer
  const [deployer] = await hre.ethers.getSigners();
  const adminAddress = process.env.ADMIN_ADDRESS || deployer.address;
  
  // Oracle address - should be set in environment variables
  // For Ethereum mainnet, use Chainlink CNY/USD feed
  const oracleAddress = process.env.CHAINLINK_CNY_USD_ETH;
  
  if (!oracleAddress || oracleAddress === "0x0000000000000000000000000000000000000000") {
    console.error("Error: Please set CHAINLINK_CNY_USD_ETH in .env file");
    process.exit(1);
  }
  ...
  console.log("CNY Stablecoin deployed to:", stablecoin.address);
